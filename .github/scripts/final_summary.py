"""
12 AM Final Summary — runs inside GitHub Actions
------------------------------------------------------
...
If nothing happened that day:
  ...
  - Does NOT touch the Google Sheet at all
"""
...
def get_yesterday_range_utc():
    """
    This job runs right at the PKT day boundary (~00:00 PKT). We want to
    summarize the day that JUST ENDED, so we compute the full previous
    Karachi calendar day and convert that range to UTC for git log.
    """
...
def parse_problems(commits):
    """
    Returns a dict keyed by problem number, keeping the LATEST commit for
    each problem that day (git log lists newest-first, so the first match
    we see per number is the most recent state).
    """
    problems = {}
    for commit in commits:
        ...
        action, number, title = match.group(1), match.group(2), match.group(3)
        if number in problems:
            continue  # already have the latest state for this problem today

        solution_file = next(
            (f for f in commit["files"] if PROBLEMS_SUBDIR in f and not f.endswith("README.md")),
            None,
        )
        if not solution_file:
            continue

        topic = solution_file.split(f"{PROBLEMS_SUBDIR}/")[-1].split("/")[0]
        ...
        problems[number] = {
            ...
            "topic": topic,
            "url": github_url,
            "action": action,  # "Solve" or "Resubmit"
        }
...
def upsert_problems_to_sheet(problems):
    ...
    date_str = datetime.now(KARACHI).strftime("%Y-%m-%d")
    ...
    for p in problems:
        ...
        row_values = [date_str, day_str, p["number"], link_formula, p["topic"]]
        ...
        if existing_row_index:
            ...
            print(f"  Updated existing row for {p['number']}. {p['title']}")
        else:
...
def build_summary_email(problems):
    solves = [p for p in problems if p["action"] == "Solve"]
    resubmits = [p for p in problems if p["action"] == "Resubmit"]

    lines = ["Hi,\n"]
    lines.append(f"Here's your LeetCode summary for today — {len(problems)} problem(s) touched:\n")

    for p in problems:
        tag = " (resubmitted with an updated solution)" if p["action"] == "Resubmit" else ""
        lines.append(f"- {p['number']}. {p['title']} — {p['topic']}{tag}")

    lines.append("\nGreat consistency today — keep the streak going!\n")
    lines.append("— Your LeetCode Automation")

    subject = f"LeetCode: {len(solves)} solved, {len(resubmits)} updated today" if resubmits \
        else f"LeetCode: {len(solves)} problem(s) solved today"
...
def main():
    ...
    if problems:
        ...
        print(f"Emailed and logged {len(problems)} problem(s).")
    else:
        body = (
            "Hi,\n\n"
            "No LeetCode problems were solved or pushed to GitHub today. "
            "Your tracking sheet has not been changed.\n\n"
            "Tomorrow's a fresh start — good luck!\n\n"
            "— Your LeetCode Automation"
        )
        send_email("LeetCode: No Submission Today", body)
        print("No activity today — 'no submission' email sent, Sheet untouched.")
